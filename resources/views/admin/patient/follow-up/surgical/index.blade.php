        ...
        <div class="ui segments">
            ...
            <div class="ui black segment">
                <table class="ui compact celled definition table">
                    ...
                    <tbody ng-cloak>
                        <tr ng-repeat="followup in surgicalFollowup track by $index" ng-click="setSelected();" ng-class="{'bg-info lt': followup.id === selected.id}">
                            <td>@{{ followup.date }}</td>
                            <td>@{{ followup.complain }}</td>
                            <td>@{{ followup.examination }}</td>
...
@section('script')
    <script>
        app.controller('FollowUpController', ['$scope', '$http', function ($scope, $http) {
            $scope.moduleUrl = "{{ route('surgical.followup.index', ['patient' => $patient]) }}";
            ...
            $http.get($scope.moduleUrl + '?ajax=true').then(function (response) {
                $scope.surgicalFollowup = response.data;
            });
